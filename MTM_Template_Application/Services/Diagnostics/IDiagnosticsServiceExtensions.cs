...
public interface IDiagnosticsServiceExtensions
{
    ...
    /// <summary>
    /// Gets recent error entries from the global error buffer.
    /// </summary>
    /// <param name="count">Number of errors to retrieve (max 100)</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Read-only list of recent errors, most recent first</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if count &lt; 1 or count &gt; 100</exception>
    Task<IReadOnlyList<ErrorEntry>> GetRecentErrorsAsync(int count, CancellationToken cancellationToken = default);
